...
th-3 x (⊆-def .(∪ x) .x y) =
    ...
    where lm-1 : (a b : 𝕊) → a ⊆ b → (α : 𝕊) → α ∈ a → α ∈ b
          ...
          lm-2 z (∃-def a .(λ α → z ∈ α and α ∈ 𝓟 x) (and-def b c)) = lm-1 a x ((back (𝓟-def a x)) c) z b

th-4 : ¬(∃ λ x → (y : 𝕊) → y ∈ x)
th-4 = ¬-def {!!} (λ { (∃-def x .(λ x → (y : 𝕊) → y ∈ x) z) → {!!} })
